  ...
  const processExcelData = (rows) => {
    ...
    rows.forEach((row, index) => {
      try {
        ...
        // Log de debug para kilometraje
        if (kilometraje !== 0 || horasMaquina !== 0) {
          console.log(`📊 Fila ${index + 2}: ${codigo || patente || nombre}`);
          console.log(`   KM: ${kmInicial} → ${kmFinal} = ${kilometraje} km`);
          console.log(`   Horas: ${horometroInicial} → ${horometroFinal} = ${horasMaquina}h`);
        }
        
        // Determinar si el día es contabilizable
        const fueraDeServicio = actividadesRealizadas?.toUpperCase().includes('FUERA DE SERVICIO');
        ...
        // Crear log - SI hay horas O kilometraje (o si está fuera de servicio)
        if (horasMaquina > 0 || kilometraje > 0 || fueraDeServicio) {
          logs.push({
            machineKey: machineKey,
            date: formatDate(fecha),
            productiveHours: fueraDeServicio ? 0 : horasMaquina,
            standbyHours: 0,
            downtimeHours: 0,
            kilometraje: fueraDeServicio ? 0 : kilometraje,
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-blue-800">
              ...
              <ul className="mt-2 space-y-1 ml-4 list-disc">
                ...
                <li>Actividades Realizadas (si es "Fuera de Servicio" no se contabiliza)</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="grid md:grid-cols-3 gap-4 mb-4">
            <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl">
              <div className="text-sm font-semibold text-emerald-600">Equipos Nuevos</div>
              <div className="text-2xl font-black text-emerald-700">{preview.machines.size}</div>
            </div>
            <div className="p-4 bg-violet-50 border border-violet-200 rounded-xl">
              <div className="text-sm font-semibold text-violet-600">Registros a Crear</div>
              <div className="text-2xl font-black text-violet-700">
                {preview.logs.filter(l => !l.fueraDeServicio).length}
              </div>
            </div>
            <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl">
              <div className="text-sm font-semibold text-amber-600">Fuera de Servicio</div>
