                    ...
                    <tr key={row.id} className="transition-colors" style={{}} onMouseEnter={e => e.currentTarget.style.background = "#faf9ff"} onMouseLeave={e => e.currentTarget.style.background = ""}>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          {/* Botón documento: si tipo=otro y tiene URL propia, abre ese archivo; si no, genera PDF FleetCore */}
                          {row.tipo === 'otro' && row.urlDocumento ? (
                            <a href={row.urlDocumento} target="_blank" rel="noreferrer"
                            ...
                            <button onClick={() => generarPDFAnexo(row, row._contrato, row._trabajador, row._nro)} className="p-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-600 rounded-lg transition-colors" title="Vista previa PDF">
                ...
                {paginados.map(row => {
                  ...
                  return (
                    <tr key={row.id} className="transition-colors" style={{}} onMouseEnter={e => e.currentTarget.style.background = "#faf9ff"} onMouseLeave={e => e.currentTarget.style.background = ""}>
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs font-black px-2 py-0.5 rounded-full ${row.tasaPct === 0 ? 'bg-slate-100 text-slate-400' :
                  ...
                  {tramosConPesos.map((t, i) => (
                    <tr key={i} className={`${i % 2 === 0 ? 'bg-slate-50' : ''}`}>
                      ...
                      <td className="px-3 py-2">
                        <span className={`font-black px-2 py-0.5 rounded-full text-xs ${t.tasa === 0 ? 'bg-slate-100 text-slate-500' :
        ...
        <div className="flex border-b border-slate-100">
          {INNER_TABS.map(t => (
            <button key={t.id} onClick={() => setTabInner(t.id)}
              className={`flex-1 px-4 py-3.5 text-sm font-bold transition-colors border-b-2 -mb-px ${tabInner === t.id
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: "#fff", border: "1px solid rgba(0,0,0,0.06)", boxShadow: "0 2px 8px rgba(0,0,0,0.05), 0 8px 24px rgba(124,58,237,0.04)" }}>
        <div className="flex border-b border-slate-100 overflow-x-auto">
          {INNER_TABS.map(t => (
            <button key={t.id} onClick={() => setTabInner(t.id)}
              className={`flex-shrink-0 px-5 py-3.5 text-sm font-bold transition-colors border-b-2 -mb-px whitespace-nowrap ${tabInner === t.id
                  ? 'border-violet-600 text-violet-700 bg-violet-50/60'
                  : 'border-transparent text-slate-400 hover:text-slate-600 hover:bg-slate-50/80'
                }`}>{t.label}</button>
          ))}
