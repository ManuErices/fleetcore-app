...
function ContratosSection() {
  ...
  const handleDelete = async () => {
    try { await deleteDoc(doc(db, 'empresas', empresaId, 'contratos', confirm.id)); load(); }
    catch (e) { alert('Error: ' + e.message); }
    setConfirm(null);
  };
...
function RemuneracionesSection() {
  ...
  // Licencias que caen en el período, con sus días ya derivados de las fechas.
  // Mismo criterio que los anticipos: si hay registro manda sobre el campo
  // manual de la liquidación, porque sumar ambos descontaría dos veces los
  // mismos días de reposo.
  const licenciasDelPeriodo = useCallback((trabajadorId, mes, anio) => {
    const ls = licenciasDe(licencias, trabajadorId, mes, anio);
  ...
  const handleDelete = async () => {
    try { await deleteDoc(doc(db, 'empresas', empresaId, 'remuneraciones', confirm.id)); load(); }
    catch (e) { alert('Error: ' + e.message); }
    setConfirm(null);
  };
  ...
  const enriquecidas = liquidaciones.map(l => {
    ...
    const licenciasReg = licenciasDelPeriodo(l.trabajadorId, l.mes, l.anio);
    const calc = contrato
      ? liquidacionDe(trabajador, contrato, l, {
          anticiposRegistrados: anticipoReg,
          licenciasRegistradas: licenciasReg,
        })
      : null;
    return {
      ...l, _trabajador: trabajador, _contrato: contrato, _calc: calc,
      _anticipoReg: anticipoReg, _licenciasReg: licenciasReg,
  ...
  return (
    <>
                ...
                {paginadas.map(row => {
                  ...
                  return (
                    <tr key={row.id} className="transition-colors" style={{}} onMouseEnter={e => e.currentTarget.style.background = "#faf9ff"} onMouseLeave={e => e.currentTarget.style.background = ""}>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <button onClick={() => generarPDFLiquidacion(row, row._trabajador, row._contrato, { empresa, anticiposRegistrados: row._anticipoReg, licenciasRegistradas: row._licenciasReg })} className="p-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-600 rounded-lg transition-colors" title="Descargar liquidación PDF">
                            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                          </button>
                          <button onClick={() => openEdit(row)} className="p-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-600 rounded-lg transition-colors" title="Editar">
                            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
      ...
      {/* La previsualización del modal tiene que calcular con las MISMAS fuentes
          que la tabla y la nómina. Sin esto, el modal mostraba el anticipo
          escrito a mano mientras el resto del sistema usaba el de la colección,
          y los dos líquidos no coincidían. */}
      <LiquidacionModal isOpen={modal} onClose={() => setModal(false)} editData={editData}
        trabajadores={trabajadores} contratos={contratos}
        anticiposRegistrados={editData
          ? anticiposDelPeriodo(editData.trabajadorId, editData.mes, editData.anio)
          : undefined}
        licenciasRegistradas={editData
          ? licenciasDelPeriodo(editData.trabajadorId, editData.mes, editData.anio)
          : undefined}
        onSaved={load} />
      ...
      <ReliquidacionModal
        ...
        licenciasRegistradas={(reliq?.original || reliq?.editData)?._licenciasReg}
        onSaved={load}
      />
      <CargaMasivaModal
        isOpen={cargaMasiva}
...
function FiniquitosSection() {
  ...
  const handleDelete = async () => {
    try { await deleteDoc(doc(db, 'empresas', empresaId, 'finiquitos', confirm.id)); load(); }
    catch (e) { alert('Error: ' + e.message); }
    setConfirm(null);
  };
