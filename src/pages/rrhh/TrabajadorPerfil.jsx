...
export default function TrabajadorPerfil() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [cSnap, rSnap, aSnap, asSnap, auSnap] = await Promise.all([
        ...
        getDocs(query(collection(db, 'empresas', empresaId, 'ausencias'),    orderBy('fechaDesde', 'desc'))),
      ]);
      ...
      setAusencias(auSnap.docs.map(d => ({ id: d.id, ...d.data() })).filter(au => au.trabajadorId === trabajador.id));
    ...
    <div className="min-h-screen" style={{ background: '#f0f2f7' }}>
        ...
        <div className="w-72 flex-shrink-0 space-y-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 grid grid-cols-2 gap-3">
            {[
              ...
              { label: 'Ausencias',   value: ausencias.length,    color: 'text-red-500' },
                ...
                {tab === 'ausencias' && (
                  <div className="p-5 space-y-5">
                    <div className="flex justify-between items-center flex-wrap gap-2">
                      <div>
                        <h3 className="text-sm font-black text-slate-800">Historial de Ausencias y Licencias</h3>
                        <p className="text-xs text-slate-400 mt-0.5">Control administrativo de inasistencias y permisos</p>
                      </div>
                      <button
                        onClick={() => { setEditingAusencia(null); setShowAusenciaModal(true); }}
                        className="flex items-center gap-1.5 px-4 py-2 bg-purple-600 text-white font-bold text-xs rounded-xl hover:bg-purple-700 transition-colors shadow-sm"
                      >
                    ...
                    {ausencias.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-16 text-slate-400 rounded-2xl bg-slate-50 border border-slate-100">
                        <svg className="w-10 h-10 opacity-25 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                        </svg>
                        <p className="font-semibold text-sm">Sin ausencias o licencias registradas</p>
                        <p className="text-xs mt-1 text-slate-400">Las ausencias registradas justificarán las marcas en el registro de asistencia.</p>
                      </div>
                    ) : (
                          ...
                          <table className="w-full text-left text-sm border-collapse">
                            <thead>
                              <tr className="bg-slate-50 border-b border-slate-200">
                                ...
                                <th className="p-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Fecha Desde</th>
                                <th className="p-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Días / Horas</th>
                                <th className="p-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Motivo / Licencia</th>
                                <th className="p-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Médico</th>
                                <th className="p-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Observaciones</th>
                                <th className="p-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Acciones</th>
                              </tr>
                            </thead>
                                  ...
                                  <tr key={a.id} className="hover:bg-slate-50/50">
                                    <td className="p-3">
                                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold capitalize ${
                                        a.tipo === 'sin goce' ? 'bg-red-50 text-red-700' :
                                        a.tipo === 'permiso con goce' ? 'bg-emerald-50 text-emerald-700' :
                                        a.tipo?.startsWith('licencia') ? 'bg-amber-50 text-amber-700' :
                                        'bg-slate-50 text-slate-600'
                                      }`}>
                                        {a.tipo}
                                      </span>
                                    </td>
                                    <td className="p-3 font-medium text-slate-700 text-xs">
                                      {a.fechaDesde ? new Date(a.fechaDesde + 'T00:00:00').toLocaleDateString('es-CL') : '—'}
                                    </td>
                                    <td className="p-3 text-slate-600 text-xs">
                                      {a.medioDia ? 'Medio día' : `${a.dias} día(s)`}
                                      {a.horaDesde && (
                                    ...
                                    <td className="p-3 text-xs">
                                      <div className="font-semibold text-slate-700">{a.motivo || '—'}</div>
                                      {isLic && a.nroLicencia && (
                                        <div className="text-[10px] text-slate-400 mt-0.5">
                                    ...
                                    <td className="p-3 text-xs text-slate-500 max-w-xs truncate" title={a.observaciones}>
                                      {a.observaciones || '—'}
                                    </td>
                                    <td className="p-3">
                                      <div className="flex items-center gap-1.5">
                                        <button
                                          onClick={() => { setEditingAusencia(a); setShowAusenciaModal(true); }}
                                          className="p-1 text-slate-400 hover:text-violet-600 hover:bg-slate-100 rounded-lg transition-all"
                                          title="Editar"
                                        >
                                          {Ico.pencil}
                                        </button>
                                        <button
                                          onClick={async () => {
                                            ...
                                            try {
                                              ...
                                              await deleteDoc(doc(db, 'empresas', empresaId, 'ausencias', a.id));
                                              load();
                                            } catch (e) {
      ...
      {showAusenciaModal && (
        <AusenciaModal
          ...
          preselectedTrabajadorId={trabajador.id}
